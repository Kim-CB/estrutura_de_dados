...
class TestExerc3(unittest.TestCase):
    def setUp(self):
        # Configuração do ambiente teste
        self.arq_out = open("temp.txt", "w+")
        self.arq_resposta = open("out3.txt", "r", encoding="utf8")
    ...
    def test_milhao_linhas_tam(self):
        ...
        print("\nTeste tamanho saída milhão de linhas OK", file=sys.stderr)
    ...
    def test_milhao_linhas_conteudo(self):
        ...
        for linha in self.arq_resposta:
            self.assertEqual(linha, self.arq_out.readline(), "Conteúdo diferente do esperado")
        print("\nTeste de conteúdo saída milhão de linhas OK")
