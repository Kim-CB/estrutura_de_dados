...
class BaseSet(BaseCollection):
    ...
    def __in__(self, x):
        # Teste se elemento x está na estrutura
...
class BaseList(BaseCollection):
    # Classe de Base para implementações de lista 
    def append(self, x):
    ...
    def clear(self):
        # Pode ser anulado com implementações mais eficientes
        while self.size() > 0:
    ...
    def __iter__(self):
        # Esse implementação é suficiente para listas array-based
    ...
    def index(self, x):
        ...
        for y in self:
            if y == x:
                return i
            i += 1
        raise ValueError("%r não está na lista" % x)
