...
class USet():
    ...
    # Acrescenta x ao conjunto desde que não exista nenhum elemento y no conjunto de tal modo que x seja igual a y.
    # Retorna true se x foi acrescentado ao conjunto e false caso contrário.
    def add(self, x):
    ...
    # Encontra um elemento y no conjunto de modo que x seja igual a y e remove y.
    ...
    # Encotra um elemento y no conjunto de modo que y seja igual a x.
    # Retorna y, ou None se tal elemento não existe.
    def find(self, x):
        if x in self.conjunto:
            return x
